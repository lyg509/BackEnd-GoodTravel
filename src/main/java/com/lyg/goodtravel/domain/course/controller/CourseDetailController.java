...
public class CourseDetailController {
    ...
    public ResponseEntity<CourseDetailGetRes> courseDetailUser(
        ...
        if (courseDetailList != null
                && !courseDetailList.isEmpty()
                && courseTouristDetailList != null
                && !courseDetailList.isEmpty()) {
            return ResponseEntity
                    .status(200)
                    .body(CourseDetailGetRes.of(200, "Success",
                            courseDetailList, courseTouristDetailList));
        } else {
            log.error("courseId doesn't exist");
            return ResponseEntity
                    .status(403)
                    .body(CourseDetailGetRes.of(403, "courseId doesn't exist",
                            null, null));
        }
    }
    ...
    public ResponseEntity<CourseRecordDetailGetRes> courseRecordDetail(
        ...
        if (courseRecordDetailList != null && !courseRecordDetailList.isEmpty()) {
            return ResponseEntity
                    .status(200)
                    .body(CourseRecordDetailGetRes.of(200,
                            "Success", courseRecordDetailList));
        } else {
            return ResponseEntity
                    .status(200)
                    .body(CourseRecordDetailGetRes.of(200,
                            "Record doesn't exist", null));
        }
    }
    ...
    public ResponseEntity<CourseTourTestResultDetailGetRes> courseTourTestDetail(
            @ApiParam(value = "코스 구분 번호") @PathVariable("courseId") int courseId) {
        ...
        if (courseTourTestResultList != null && !courseTourTestResultList.isEmpty()) {
            return ResponseEntity
                    .status(200)
                    .body(CourseTourTestResultDetailGetRes.of(200,
                            "Success", courseTourTestResultList));
        } else {
            return ResponseEntity
                    .status(200)
                    .body(CourseTourTestResultDetailGetRes.of(200,
                            "Test Result doesn't exist", null));
        }
    }
    ...
    public ResponseEntity<CourseTagDetailGetRes> courseTagDetail(
        ...
        if (courseTagDetailList != null && !courseTagDetailList.isEmpty()) {
            return ResponseEntity
                    .status(200)
                    .body(CourseTagDetailGetRes.of(200,
                            "Success", courseTagDetailList));
        } else {
            return ResponseEntity
                    .status(200)
                    .body(CourseTagDetailGetRes.of(200,
                            "Tag doesn't exist", null));
        }
    }

    @ApiOperation(value = "새로운 인연을 만날 확률(%)")
    @GetMapping("/connection/{courseId}")
    public ResponseEntity<CourseConnectionGetRes> courseConnectionDetail(
            @ApiParam(value = "코스 구분 번호") @PathVariable("courseId") int courseId) {
        ...
        if (percentage != 0) {
            return ResponseEntity
                    .status(200)
                    .body(CourseConnectionGetRes.of(200, "Success", percentage));
        } else {
            return ResponseEntity
                    .status(200)
                    .body(CourseConnectionGetRes.of(200, "Result doesn't exist", 0));
        }
    }
